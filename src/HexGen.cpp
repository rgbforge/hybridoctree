...
void hexGen::InitializeOctree(const char* inputFileName, const char* outputFileName) {
    ...
    if (ext == "raw") {
        ...
        ReadStlData(inputFileName, outputFileName);
    } else {
        std::cerr << "error: unsupported file" << ext << std::endl;
    }
}

inline void hexGen::ReadStlData(const char* inputFileName, const char* outputFileName) {
    if (!ReadStl(inputFileName, triMesh)) {
        std::cerr << "error reading STL" << std::endl;
        return;
    }
    //x,y,z,min,max
    double box[3][2]; 
    ...
    if(triMesh.r == nullptr) triMesh.r = new double[triMesh.vNum];
    for (int i = 0; i < triMesh.vNum; i++) triMesh.r[i] = 0;
    for (int j = 0; j < triMesh.eNum - 1; j++) {
        for (int k = 0; k < 3; k++) {
            for (int l = j + 1; l < triMesh.eNum; l++) {
                for (int m = 0; m < 3; m++) {
                    if (triMesh.e[l][m] == triMesh.e[j][k]) {
                        if (triMesh.e[j][(k + 1) % 3] == triMesh.e[l][(m + 1) % 3]) { publicIdx[0] = 1; publicIdx[1] = 1; }
                        else if (triMesh.e[j][(k + 1) % 3] == triMesh.e[l][(m + 2) % 3]) { publicIdx[0] = 1; publicIdx[1] = 2; }
                        else if (triMesh.e[j][(k + 2) % 3] == triMesh.e[l][(m + 1) % 3]) { publicIdx[0] = 2; publicIdx[1] = 1; }
                        else if (triMesh.e[j][(k + 2) % 3] == triMesh.e[l][(m + 2) % 3]) { publicIdx[0] = 2; publicIdx[1] = 2; }
                        else continue;
